...
#########################################
my $name = "Gauge";
my $email = "hartwelg@oregonstate.edu";
...
sub clean{
	...
	foreach my $title (@tracks) { 
		...
		## (letters, numbers, apostrophe allowed)
		##########################
		# skip title if contains any single non-English character
		# <your code here>
		next if ($title =~ /[^a-zA-Z0-9'\s]/);
		########################## End Task 5
...
sub build_bigrams{
	...
	{
		my $curword = 1;
		my @wordarray = split /\s+/, $title;
		my $length = scalar @wordarray;
		if ($length > 1)
		{
			while ($curword < $length)
			{
				if ($wordarray[$curword - 1] ne '' && $wordarray[$curword] ne '' && $wordarray[$curword - 1] !~ /\s/ && $wordarray[$curword] !~ /\s/)
				{
					$counts{$wordarray[$curword - 1]}{$wordarray[$curword]}++;
				}
				$curword++;
			}
		}
...
sub mcw{
	...
	##                      ##   
	##########################
		my @keys = sort {$counts{$word}{$b} <=> $counts{$word}{$a} or $a cmp $b} keys %{$counts{$word}};
		foreach my $key (@keys)
		{
			$best_word = $key;
			last if ($word_history{$best_word} != 1);
		}
	########################## End Task MCW
...
sub sequence{
	...
	my $songTitle = '';
	my $nextword = '';
	$songTitle = $songTitle . $_[0];
	$word_history{$_[0]} = 1;
	my @words = split /\s+/, $songTitle;
	#my $length = scalar @words;
	my $length = 0;
	while ($length < $SEQUENCE_LENGTH)
	{
		$length = scalar @words;
		$nextword = mcw($words[$length - 1]);
		last if ($nextword eq /\s+/);
		$songTitle = $songTitle . ' ' . $nextword;
		$word_history{$nextword} = 1;
		@words = split /\s+/, $songTitle;
	}
	if ($length < $SEQUENCE_LENGTH || $nextword eq '')
	{
		$songTitle = $songTitle . ' ';
	}
	# return the sequence you created instead of this measly string
	return $songTitle;
	########################## End Task Song Title
}
